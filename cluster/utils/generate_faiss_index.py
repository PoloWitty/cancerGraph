"""
Description: This file generates a Faiss index from a pretrained BERT model. It uses the BERT model to generate embeddings for a list of phrases, and then uses Faiss to generate a KNN index for those embeddings. The index is saved to disk for later use.
"""
...
def get_bert_embed(phrase_list, m, tok, normalize=True, summary_method="CLS", tqdm_bar=False):
    """
    Generates BERT embeddings for a list of phrases.
    
    Args:
        phrase_list (list): List of phrases to generate embeddings for.
        m (torch.nn.Module): BERT model to use for generating embeddings.
        tok (transformers.tokenization_utils.PreTrainedTokenizer): Tokenizer to use for encoding phrases.
        normalize (bool, optional): Whether to normalize the embeddings. Defaults to True.
        summary_method (str, optional): Method to use for summarizing the embeddings. Options are "CLS" or "MEAN". Defaults to "CLS".
        tqdm_bar (bool, optional): Whether to show a progress bar. Defaults to False.
    
    Returns:
        np.ndarray: Array of BERT embeddings for the phrases.
    """
    m = m.to(device)
    input_ids = []
...
def get_KNN(embeddings, k, gpu_index, use_multi_gpu=True, exact=True):
    """
    Generates a Faiss index for a given set of embeddings.
    ...
    Parameters:
        ...
        use_multi_gpu (bool): Whether to use multiple GPUs or not.
        exact (bool): Whether to use exact search or not.
    
    Returns:
        similarity (np.array[float]): A numpy array of similarities.(n,k) n is the vocabulary length
        indices (np.array[int]): A numpy array of indices. (n,k)
    """
    if not exact:
...
def find_new_index(args):
    """
    Generates a Faiss index from a pretrained BERT model.
    
    Args:
        args (dict): A dictionary containing the following keys:
            model_name_or_path (str): The name or path of the pretrained BERT model.
            phrase2idx_path (str): The path to the phrase2idx file.
            embedding_path (str): The path to the embedding file.
            topk (int): The number of nearest neighbors to find.
            gpu_index (int): The index of the GPU to use.
            use_multi_gpu (bool): Whether to use multiple GPUs.
            exact_knn (bool): Whether to use exact KNN.
