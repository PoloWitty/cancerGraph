...
def get_sim(terms_list, similarity):
    """
    This function takes in a list of terms and a similarity matrix and returns a similarity matrix of the terms in the list.
    
    Parameters:
        terms_list (list): A list of terms.
        similarity (np.array): A similarity matrix.
    
    Returns:
        sim (np.array): A similarity matrix of the terms in the list.
    """
...
def ratio_cut(terms_list, similarity):
    """
    This function performs a ratio cut on a given list of terms and similarity matrix. It returns two clusters of terms.
    
    Parameters:
        terms_list (list): List of terms.
        similarity (matrix): Similarity matrix of the terms.
    
    Returns:
        clu0 (list): List of terms in the first cluster.
        clu1 (list): List of terms in the second cluster.
    """
    ...
    feat = feat / feat_norm
    cluster = KMeans(n_clusters=2,n_init=10).fit_predict(feat)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=50,
        type=int,
        help='the max num of nodes in a cluster'
    )
    ...
    print('there are '+str(len(need_cluster_list))+' clusters that need to be re-cluster')
    print(f'will cut avg len of {np.mean(need_cluster_length_list)} to {MAX_CLUSTER_COUNT}')

    threshold_list = [0.60]
    for threshold in threshold_list:
        print('threshold=', threshold)
        final_res = []
        for key in tqdm(cluster_res,desc='re-clustering'):
        ...
        with open('./result/final_cluster_res.txt', 'w') as f:
            for cluster in tqdm(final_res,desc='write cluster to file'):
                print_cluster_to_file(f, cluster)
                # f.write('-------')
                # f.write('RATIO\n')
